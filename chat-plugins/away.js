...
exports.commands = {
	away: function (target, room, user) {
		...
		if (statusLen > 16) return this.sendReply("Your away status should be short and to-the-point, not a dissertation on why you are away.");
		...
		newName += ' - ' + status;
		if (newName.length > 20) return this.sendReply("\"" + target + "\" is too long to use as your away status.");
		...
		if (user.can('lock', null, room)) this.add("|raw|-- <font color='" + color(user.userid) + "'><strong>" + Tools.escapeHTML(user.name) + "</strong></font> is now " + target + ".");
	...
	back: function (target, room, user) {
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('lock', null, room)) this.add("|raw|-- <font color='" + color(user.userid) + "'><strong>" + Tools.escapeHTML(newName) + "</strong></font> is no longer " + status + ".");
	},
