...
var core = exports.core = {
    ...
    profile: {
        ...
        about: function (user) {
            return Core.stdin('db/status', user);
        },
        ...
        tournamentElo: function (user) {
            var elo = Core.stdin('db/elo', user);
            if (elo === 0) {
                return 1000;
            }
        ...
        rank: function (user) {
            var data = fs.readFileSync('config/db/elo.csv', 'utf-8');
            var row = ('' + data).split("\n");
    ...
    ladder: function (limit) {
        ...
        if (list.length > 1) {
            ...
            while (len--) {
                ladder = ladder + '<tr><td>' + (list.length - len) + '</td><td>' + list[len][0] + '</td><td>' + Math.floor(list[len][1]) + '</td><td>' + this.stdin('db/tourWins', list[len][0]) + '</td></tr>';
                if (len === limit) break;
            }
            ladder += '</tbody></table>';
            return ladder;
