...
class Tournament {
	...
	onTournamentEnd() {
		...
		if (this.room.isOfficial && tourSize >= sizeRequiredToEarn) {
			...
			this.room.addRaw(Util.nameColor(winner, true, true) + " has won " + "<strong><font color='red'>" + firstMoney + "</font></strong>" + currencyName(firstMoney) + " for winning the tournament!</b>");

			if (runnerUp) {
				Db.money.set(rid, Db.money.get(rid, 0) + secondMoney);
				this.room.addRaw(Util.nameColor(runnerUp) + " has won " + "<strong><font color='red'>" + secondMoney + "</font></strong>" + currencyName(secondMoney) + " for winning the tournament!</b>");
			}
		}
